const SUPABASE_URL = "https://qrecvptbfbaklpbmajfg.supabase.co";
const SUPABASE_KEY = "sb_publishable_H7K0M9XkuDkas8sTum_KMQ_92wjBoul";
...
async function getAIAdvice(){
  ...
  try{
    ...
    document.getElementById('ai-body').textContent = data.text;
...
async function doLogin(){
  ...
  if(error){
  err.textContent = error.message; // 🔥 show real error
  err.classList.add('show');
...
if(!existingUser){
  await client.from('profiles').insert([
    {
      ...
      monthly_income: data.user.user_metadata?.monthly_income || 0
    }
  ]);
}
  ...
  monthlyIncome: data.user.user_metadata?.monthly_income || 0
};
...
async function doSignup(){
  ...
  // ✅ Save extra user info locally (for now)
...
async function enterApp(){
  ...
  const currentPage = localStorage.getItem('currentPage') || 'pg-dashboard';

  pdfBtn.style.display = currentPage === 'pg-reports' ? 'flex' : 'none';
}, 0);
}
...
function updateUserUI(){
  ...
  // ✅ handle both normal + google users
  ...
  document.getElementById('topbar-sub').textContent = greet+', '+name+' 👋';
}
...
if(pdfBtn){
  pdfBtn.style.display = pageId === 'pg-reports' ? 'flex' : 'none';
}
if(pageId === "pg-reports"){
  getReportAI();
}
}
...
async function exportPDF(){
  ...
  // 🤖 AI Insights
  let aiSummary = "Your spending is under control.";
  let aiRecommendation = "Keep maintaining your current financial habits.";

  if(expense > income){
    aiSummary = "You are spending more than your income.";
    aiRecommendation = "Reduce unnecessary expenses immediately.";
  } else if(savings > 0){
    aiSummary = `You saved ₹${savings} this month.`;
    aiRecommendation = "You can start investing small amounts.";
  }

  const element = document.createElement("div");

  // 📊 FIXED CHART CAPTURE (YOUR ID)
  let chartHTML = "";
  const chartWrap = document.getElementById("pie-chart-wrap");

  if(chartWrap){
    chartHTML = chartWrap.outerHTML;
  } else {
    chartHTML = "<p>Chart not available</p>";
  }
  ...
  element.innerHTML = `
    <div style="font-family: Arial; padding: 25px; color:#1e293b;">
      ...
      <!-- SUMMARY -->
      <h3 style="color:#334155;">Financial Summary</h3>
      <p>Total Income: ₹${income}</p>
      <p>Total Expenses: ₹${expense}</p>
      <p>Net Savings: ₹${savings}</p>

      <!-- CHART -->
      <h3 style="color:#334155; margin-top:20px;">Spending Overview</h3>
      <div style="margin-bottom:20px;">
        ${chartHTML}
      </div>

      <!-- AI -->
      <h3 style="color:#334155;">AI Insights</h3>
      ...
      <table style="width:100%; border-collapse: collapse;">
        ...
        ${state.transactions.map(t=>`
          <tr>
            ...
            <td style="padding:8px; border:1px solid #ccc;">${t.category || t.source}</td>
            <td style="padding:8px; border:1px solid #ccc;">₹${t.amount}</td>
            <td style="padding:8px; border:1px solid #ccc;">${t.date}</td>
          </tr>
        `).join("")}
      </table>
...
async function saveIncome(){
  ...
  if(state.editingId){
    ...
    await updateTransaction(state.editingId, amt, source, notes);
    ...
    renderDashboard();
    showToast('Income updated ✏️');

    return;
  }

  // ✅ NORMAL ADD
  await addTransaction("income", amt, source, notes);
...
async function saveExpense(){
  ...
  if(state.editingId){
    await updateTransaction(state.editingId, amt, cat, desc);
  ...
  await addTransaction("expense", amt, cat, desc);
...
async function updateTransaction(id, amount, category, description){
...
document.getElementById('su-pass2').addEventListener('input', function(){
  const pass  = document.getElementById('su-pass').value;
...
client.auth.getSession().then(({ data }) => {
  if (data.session) {
    ...
    showAuth('page-login'); // ✅ show login if not logged in
  }
});
